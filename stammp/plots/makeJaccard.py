...
def getEntries(sites, q1, q2):
    ...
    #c = b-a
    #print(str(count)+' '+str(math.trunc(c.microseconds/1000)))
    return pc

def main(parclipA, parclipB, outfile, verbose):
    ...
    for q1 in range(len(quantiles)-1):
        ...
        for q2 in range(len(quantiles)-1):
            ...
            for j in range(bq.size()):
                if aq.exactSearch(bq.chrs[j], bq.pos[j], bq.strand[j], width=5)[1]:
                    intersect += 1
...
def run():
    ...
    parser = argparse.ArgumentParser(description='Plots Jaccard-index for two given PAR-CLIP data files.', epilog="contact: torkler@genzentrum.lmu.de")
    ...
    parser.add_argument('--zlimit', help='set maximum jaccard index for plotting [default: 1.0]', dest='zlimit', default=1.0, type=float)
    ...
    main(args.parclipA, args.parclipB, outfile, args.verbose)
    ...
    os.system('R -q --slave -f '+scriptPath+'plotJaccard.R --args '+outfile+' '+str(args.zlimit)+' '+outfile_pdf+' '+args.nameA+' '+args.nameB)
